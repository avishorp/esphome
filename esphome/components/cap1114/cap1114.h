...
enum {
  ...
  CAP1114_REG_LED_DUTY_CYCLE = 0x93
};
...
class CAP1114TouchChannel : public binary_sensor::BinarySensor {
 public:
  void set_channel(uint8_t channel) { channel_ = channel - 1; }
...
class CAP1114Component : public Component, public i2c::I2CDevice {
 public:
  ...
  void register_output_channel(CAP1114OutputChannel *channel) {
    channel->set_parent(this);
    this->output_channels_.push_back(channel);
  }
  ...
  void set_min_brightness(uint8_t value) { this->min_brightness_ = value; }
  void set_max_brightness(uint8_t value) { this->max_brightness_ = value; }
  void setup() override;
 ...
 protected:
  ...
  std::optional<uint8_t> min_brightness_;
  std::optional<uint8_t> max_brightness_;
